...
import { 
  ...
  fetchAgentTarget, 
  updateAgentTarget,
  fetchAgentDrilldown // <--- Imported drilldown API
} from "@/api/agentApi";
import { Button } from "@/components/ui/button";
import { Loader2, X } from "lucide-react"; // <--- Imported icons for Modal
    ...
    <div className="bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen relative">
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-600">Track your collections and performance metrics</p>
        </div>
            ...
            <div>
              ...
              {editingTarget ? (
                <div className="flex gap-2">
                  ...
                  />
                  <Button onClick={handleSaveTarget} disabled={savingTarget} className="bg-purple-600 hover:bg-purple-700 text-white" size="sm">
                    {savingTarget ? "Saving..." : "Save"}
                  </Button>
                  <Button onClick={() => { setEditingTarget(false); setTargetInput(agentTarget ? String(agentTarget) : ""); }} className="bg-slate-300 hover:bg-slate-400 text-slate-900" size="sm">
                    Cancel
                  </Button>
                </div>
              ) : (
        ...
        <div className="bg-white rounded-lg shadow-md p-4 mb-8">
          <p className="text-sm font-medium text-slate-700 mb-3">Time Period</p>
            ...
            {["today", "yesterday", "thisWeek", "thisMonth", "custom"].map((filter) => (
              <Button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-all ${
                  timeFilter === filter ? "bg-blue-600 text-white shadow-md" : "bg-slate-100 text-slate-700 hover:bg-slate-200"
                }`}
              >
                {filter === "today" ? "Today" : filter === "yesterday" ? "Yesterday" : filter === "thisWeek" ? "This Week" : filter === "thisMonth" ? "This Month" : "Custom"}
              </Button>
            ))}
          </div>
          ...
          {timeFilter === "custom" && (
            <div className="mt-4 flex gap-3">
              <div className="flex-1">
                ...
                <input type="date" value={customFromDate} onChange={(e) => setCustomFromDate(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
              </div>
              <div className="flex-1">
                <label className="text-xs font-medium text-slate-600 mb-1 block">To Date</label>
                <input type="date" value={customToDate} onChange={(e) => setCustomToDate(e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
              </div>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-600 mb-1">Calls Attended</p>
                <p className="text-4xl font-bold text-slate-900">{overview.calls_attended}</p>
              </div>
            ...
            <p className="text-xs text-slate-500 mt-3">Total disposition submissions</p>
          </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-600 mb-1">PTP Customers</p>
                <p className="text-4xl font-bold text-slate-900">{overview.ptp_count}</p>
              </div>
            ...
            <p className="text-xs text-purple-500 mt-3 font-medium">Click to view details &rarr;</p>
          </div>

          <div className="bg-white rounded-lg shadow-md p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate-600 mb-1">Total PTP Amount</p>
                <p className="text-3xl font-bold text-slate-900">{formatCurrency(overview.total_ptp_amount)}</p>
              </div>
            ...
            <p className="text-xs text-slate-500 mt-3">Expected to collect</p>
          </div>
        </div>

        {/* SECTION B: Collection Breakdown */}
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          <h2 className="text-xl font-bold text-slate-900 mb-6">Collection Breakdown</h2>
                ...
                >
                  ...
                  <div className="space-y-2">
                    <div>
                      ...
                      <p className="text-2xl font-bold">{item.customer_count}</p>
                    </div>
                    <div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-xl font-bold text-slate-900 mb-6">Total Collection Summary</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              <p className="text-sm font-medium text-green-700 mb-2">Total Collected Customers</p>
              <p className="text-5xl font-bold text-green-900">{summary.total_collected_count}</p>
              <p className="text-xs text-green-700 mt-2 font-medium">Click to view details &rarr;</p>
            </div>

            <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-lg p-6 border border-emerald-200">
              <p className="text-sm font-medium text-emerald-700 mb-2">Total Collected Amount</p>
              <p className="text-4xl font-bold text-emerald-900">{formatCurrency(summary.total_collected_amount)}</p>
              <p className="text-xs text-emerald-700 mt-2">Actual amount recovered</p>
            </div>
          </div>

          <div className="mt-8 pt-8 border-t border-slate-200">
            <h3 className="text-lg font-semibold text-slate-900 mb-4">Expected vs Actual</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-4 bg-amber-50 rounded-lg border border-amber-200">
                <p className="text-sm text-amber-700 font-medium">Expected to Collect (PTP)</p>
                <p className="text-3xl font-bold text-amber-900 mt-2">{formatCurrency(overview.total_ptp_amount)}</p>
              </div>

              <div className="p-4 bg-green-50 rounded-lg border border-green-200">
                <p className="text-sm text-green-700 font-medium">Actually Collected</p>
                <p className="text-3xl font-bold text-green-900 mt-2">{formatCurrency(summary.total_collected_amount)}</p>
              </div>
            </div>

            <div className="mt-6 p-4 bg-slate-50 rounded-lg border border-slate-200">
              <p className="text-sm text-slate-700 font-medium mb-3">Collection Achievement Rate</p>
              <div className="flex items-center gap-4">
                <div className="flex-1">
        ...
        {monthlySummary && (
          <div className="bg-white rounded-lg shadow-md p-6 mt-8">
            <h2 className="text-xl font-bold text-slate-900 mb-2">Monthly Summary (Calendar Month)</h2>
            <p className="text-sm text-slate-600 mb-6">This section always shows the current calendar month, independent of the selected time filter</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              ...
              >
                <p className="text-sm font-medium text-blue-700 mb-2">Customers Collected (This Month)</p>
                <p className="text-4xl font-bold text-blue-900">{monthlySummary.total_collected_count}</p>
                <p className="text-xs text-blue-700 mt-2 font-medium">Click to view details &rarr;</p>
              </div>

              <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-lg p-6 border border-emerald-200">
                <p className="text-sm font-medium text-emerald-700 mb-2">Amount Collected (This Month)</p>
            ...
            {monthlySummary.target_amount && (
              <div className="mt-6 p-6 bg-slate-50 rounded-lg border border-slate-200">
                <p className="text-sm text-slate-700 font-medium mb-4">Monthly Achievement vs Target</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                  ...
                  <div className="p-4 bg-white rounded-lg border border-slate-200">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">{formatCurrency(monthlySummary.expected_amount)}</span>
                      </div>
                      <div className="flex justify-between border-t pt-2">
                        <span className="text-slate-700">Target:</span>
                        <span className="font-semibold">{formatCurrency(monthlySummary.target_amount)}</span>
                      </div>
                      <div className="flex justify-between border-t pt-2">
                        <span className="text-slate-700">Collected (Actual):</span>
                        <span className="font-semibold text-green-700">{formatCurrency(monthlySummary.total_collected_amount)}</span>
                      </div>
                      <div className="flex justify-between border-t pt-2">
            ...
            <div className="p-6 overflow-y-auto flex-1">
              {drilldownLoading ? (
                ...
                <div className="text-center py-12 text-slate-500">
                  No customers found for this disposition in the selected timeframe.
                </div>
              ) : (
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {drilldownData.map((row, idx) => (
                        <tr key={idx} className="hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 text-slate-600">{row.contact_no || "-"}</td>
